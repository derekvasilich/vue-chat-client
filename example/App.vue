...
const { user, signOut } = toRefs(props)
async function getAuth() {
    try {
        ...
        return { currentUser, accessToken, idToken };
    } catch (e) {
        console.error('Cannot get current user: ', e)
    }    
}
const { currentUser, accessToken, idToken } = await getAuth() ?? {};
</script>
...
<template>
    ...
    <div class="container">
        ...
        <h3>Hi {{ currentUser?.signInDetails?.loginId }}</h3>
        ...
        <div class="row">
            <input id="token-input" :value="accessToken?.toString()" type="text" placeholder="Paste your JWT here…" />
